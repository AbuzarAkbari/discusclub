...
ON topic.sub_category_id = sub_category.id LEFT JOIN user ON topic.user_id = user.id LEFT JOIN image ON user.profile_img = image.id LEFT JOIN role ON user.role_id = role.id JOIN category_permission AS cp ON cp.category_id = category.id JOIN sub_category_permission AS scp ON scp.sub_category_id = sub_category.id JOIN topic_permission AS tp ON tp.topic_id = topic.id WHERE topic.id = :id AND cp.role_id = :role_id AND scp.role_id = :role_id AND tp.role_id = :role_id";
